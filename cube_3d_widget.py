...
class Cube3DWidget(QGLWidget):
    ...
    def load_obj(self, filename):
        ...
        if colors and len(colors) == len(vertices):
            ...
            # Assign default color (e.g., gray) if colors are missing or mismatched
        ...
        for face in obj["faces"]:
            for v_idx in face:
                if obj["colors"] and v_idx < len(obj["colors"]):
                    ...
                    # Assign default color if color is missing
                    alpha = 0.1 if obj.get("transparent", False) else 1.0
    ...
    def split_object(self, obj, plane_point, plane_normal):
        ...
        for face in faces:
            ...
            if np.all(face_side >= 0):
                ...
                if num_positive == 1 and num_negative == 2:
                    ...
                    neg_indices = [v_indices[i] for i in range(3) if not sides[i]]

                    # Compute intersection points on edges pos->neg1 and pos->neg2
                    intersect1 = self.interpolate_vertex(pos_idx, neg_indices[0], distances[pos_idx], distances[neg_indices[0]])
                    intersect2 = self.interpolate_vertex(pos_idx, neg_indices[1], distances[pos_idx], distances[neg_indices[1]])
                    ...
                    pos_indices = [v_indices[i] for i in range(3) if sides[i]]

                    # Compute intersection points on edges neg->pos1 and neg->pos2
                    intersect1 = self.interpolate_vertex(neg_idx, pos_indices[0], distances[neg_idx], distances[pos_indices[0]])
                    intersect2 = self.interpolate_vertex(neg_idx, pos_indices[1], distances[neg_idx], distances[pos_indices[1]])
        ...
        if new_vertices:
            ...
            # Handle colors: extend obj["colors"] with default colors for new vertices
            if obj["colors"]:
                default_color = [0.8, 0.8, 0.8]  # Default color (gray)
                obj_colors = list(obj["colors"])
                for _ in new_vertices:
                    obj_colors.append(default_color)
                obj["colors"] = obj_colors
            else:
                # If no colors exist, initialize with default colors
        ...
        # more advanced mesh processing techniques or external libraries
    ...
    def interpolate_vertex(self, idx1, idx2, d_neg, d_pos):
        ...
        """
        denominator = d_neg - d_pos
        epsilon = 1e-6  # Small value to prevent division by zero
        if abs(denominator) < epsilon:
            t = 0.5  # Assign mid-point if denominator is too small
        else:
            t = d_neg / denominator
